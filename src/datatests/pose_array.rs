...
mod tests {
    ...
    fn reads_request_header() {
        let header =
            from_slice::<HashMap<String, String>>(include_bytes!("pose_array_req.bin")).unwrap();
        assert_eq!(6, header.len());
        assert_eq!("/xax", header.get("topic").unwrap());
        assert_eq!("0", header.get("tcp_nodelay").unwrap());
        assert_eq!(
            &include_str!("pose_array_message_definition.txt").replace("\r", ""),
    ...
    fn reads_response_header() {
        let header =
            from_slice::<HashMap<String, String>>(include_bytes!("pose_array_res.bin")).unwrap();
        assert_eq!(6, header.len());
        assert_eq!("/xax", header.get("topic").unwrap());
