...
mod tests {
    ...
    fn reads_request_header() {
        let header =
            from_slice::<HashMap<String, String>>(include_bytes!("string_req.bin")).unwrap();
    ...
    fn reads_response_header() {
        let header =
            from_slice::<HashMap<String, String>>(include_bytes!("string_res.bin")).unwrap();
        assert_eq!(6, header.len());
        assert_eq!("/moo", header.get("topic").unwrap());
